...
class MainWindow:
    ...
    def _clean_up_indicators (self):
        indi_indicator = self.im.get_indicator()
        for acc in self.am.get_accounts():
            if acc.get_active() and acc.indicator is not None:
                indi_indicator.remove_indicator(acc)

    def quit_action_activate_cb (self, widget, data=None):
        self._clean_up_indicators()
        Gtk.main_quit()
